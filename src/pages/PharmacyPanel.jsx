...
import { addPrescriptionOnChain, transferBatchOnChain } from '../services/web3Service.js'
...
export function PharmacyPanel() {
  ...
  const filteredBatches = scanned
    ? batches.filter((b) => b.batchId === scanned.batchId)
    : batches
  ...
  const handleReceive = async () => {
    ...
    // Must be handed off by distributor
    if (batchData.currentOwnerRole !== 'distributor') {
      setError('Batch must be dispatched by a distributor before pharmacy can receive.')
      return
    }
    ...
    try {
      // Transfer ownership on blockchain to pharmacy
      ...
      await updateBatchStatus(batchData.id, {
        currentOwnerRole: 'pharmacy',
        currentOwnerWallet: wallet,
        status: 'Received by Pharmacy',
      })

      setSuccess(`Batch received successfully! Transaction: ${tx.hash.substring(0, 10)}...`)
      // Keep scanned context so received list still shows the batch
  ...
  const handlePrescriptionUploaded = async ({ url, prescriptionId }) => {
    if (!contract || !scanned?.batchId) {
      setError('Please scan a batch QR code first')
      return
    }
    ...
    try {
      const tx = await addPrescriptionOnChain(contract, scanned.batchId, url)
      setSuccess(`Prescription linked on-chain! Transaction: ${tx.hash.substring(0, 10)}...`)
      await fetchBatches()
    } catch (err) {
      setError(err.message || 'Failed to link prescription')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="space-y-6 p-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-slate-400">
          Receive batches from distributors and attach prescriptions for dispensed medicines.
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        {/* Prescription Upload & Batches */}
        <div className="lg:col-span-2 space-y-4">
          {/* Prescription Upload */}
          <div className="rounded-xl border border-slate-700/50 bg-slate-800/50 p-6">
            <h2 className="mb-4 text-lg font-semibold text-white">Prescription Upload</h2>
            ...
            {!wallet && (
              <div className="mb-4 rounded-lg border border-amber-500/50 bg-amber-500/10 px-4 py-3">
                <div className="flex items-center justify-between">
                  <p className="text-sm text-amber-200">Please connect your MetaMask wallet to upload prescriptions.</p>
                  <button
                    onClick={async () => {
                      setConnecting(true)
                      try {
            ...
            {user?.role !== 'pharmacy' && (
              <div className="mb-4 rounded-lg border border-amber-500/50 bg-amber-500/10 px-4 py-3 text-sm text-amber-200">
                Only pharmacy users can upload prescriptions.
              </div>
            )}

            {scanned && user?.role === 'pharmacy' && (
              <PrescriptionUpload
                batchId={scanned.batchId}
                pharmacyId={user.uid}
            ...
            {!scanned && (
              <p className="text-center text-sm text-slate-400">
                Scan a batch QR code to upload a prescription
              </p>
            )}
          </div>
          ...
          <div className="rounded-xl border border-slate-700/50 bg-slate-800/50 p-6">
            ...
            {loadingBatches ? (
              ...
              <p className="py-8 text-center text-sm text-slate-400">
                {scanned ? 'No received record for this scanned batch yet' : 'Scan a batch to view it here'}
              </p>
            ) : (
